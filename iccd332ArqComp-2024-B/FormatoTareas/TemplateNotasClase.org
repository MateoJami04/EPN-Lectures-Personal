...
** Notas clase, comandos usados:
- ~C-c-C-l~ Sirve para insertar un enlace y poder ponerle un nombre
personalizado, que sea como guía del enlace.
- ~M-x-: ispell-region~ Sirve para comprobar la ortografía de una
  región seleccionada.
- ~M-x: gpetel-menu~ Sirve para abrir el Copilot asociado a
  gitHub. Pero este no se abre en modo agente, si no como la última
  programación de la LLM.
- Se debe terminar de configurar el Copilot para el emacs. 
